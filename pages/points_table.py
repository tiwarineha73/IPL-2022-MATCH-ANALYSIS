...
def render(df: pd.DataFrame):
    ...
    def row_style(row):
        if row["Pos"] <= 4:
            return ["background-color: rgba(255,107,53,0.18)"] * len(row)
        return [""] * len(row)
    ...
    colors = [ACCENT if v >= 0 else "#555" for v in nrr_sorted["NRR"]]
    ...
    fig2.add_hline(y=0, line_color="#8899AA", line_width=1)
    ...
    st.markdown("""
    <div style='background:#1E2130;border-radius:12px;padding:22px 28px;
                font-size:14px;line-height:2.2;'>
        <b style='color:#8899AA;font-size:12px;letter-spacing:1px;'>QUALIFIER 1</b><br>
        Gujarat Titans vs Rajasthan Royals
        → <span style='color:#FF6B35;font-weight:700;'>Gujarat Titans ✅ (advanced to Final)</span><br><br>
        <b style='color:#8899AA;font-size:12px;letter-spacing:1px;'>ELIMINATOR</b><br>
        Lucknow Super Giants vs Royal Challengers Bangalore
        → <span style='color:#FF6B35;font-weight:700;'>RCB ✅</span><br><br>
        <b style='color:#8899AA;font-size:12px;letter-spacing:1px;'>QUALIFIER 2</b><br>
        Rajasthan Royals vs RCB
        → <span style='color:#FF6B35;font-weight:700;'>Rajasthan Royals ✅ (advanced to Final)</span><br><br>
        <hr style='border-color:#333;'/>
        <b style='color:#8899AA;font-size:12px;letter-spacing:1px;'>FINAL · Narendra Modi Stadium, Ahmedabad</b><br>
        Gujarat Titans vs Rajasthan Royals<br>
        🏆 <span style='color:#FF6B35;font-size:20px;font-weight:800;'>
            Gujarat Titans WON by 7 wickets
        </span><br>
        <span style='color:#8899AA;font-size:12px;'>Player of the Match: HH Pandya</span>
    </div>
    """, unsafe_allow_html=True)

    # ── Dataset wins vs points validation ────────────────────────────
    ...
    matches.columns = ["Team", "DataMatches"]
